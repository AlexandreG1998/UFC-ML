...
links_lutas_por_evento = []
lutas = []

for i in range(len(events)):

    print("Iniciando link: " + str(i)+ " de: " + str(len(events)))
...
for i in range(len(lutas)):
    soup_ = BeautifulSoup(lutas[i], 'html.parser')
    tabela_links2 = soup_.find_all('a')
    links_lutas_por_evento_ = []
    if(len(tabela_links2) > 0):
        for k in range(len(tabela_links2)):
            txt = tabela_links2[k].prettify()
            if(txt.find("http://www.ufcstats.com/fight-details/") >= 0):
                txt = txt.split('href=')
                txt = txt[1]
                txt = txt.split('>\n')
                txt = txt[0]
                ...
                links_lutas_por_evento_.append(txt)
        links_lutas_por_evento.append(links_lutas_por_evento_)

for i in range(len(links_lutas_por_evento)):
    index = i

    for k in range(len(links_lutas_por_evento[index])):
        links_lutas_por_evento[index][k] = links_lutas_por_evento[index][k].replace('"', '')
        links_lutas_por_evento[index][k] = links_lutas_por_evento[index][k].replace(' ', '')
events_ = []
for i in range(len(events)):
    events_.append(events[i].split('event-details/')[1])
...
for i in range(len(events_)):
    evento_correto = events_[i]
    lutas_deste_evento = links_lutas_por_evento[i]

    for k in range(len(lutas_deste_evento)):
        link_luta_.append(lutas_deste_evento[k])
        evento_correspondente.append(evento_correto)
